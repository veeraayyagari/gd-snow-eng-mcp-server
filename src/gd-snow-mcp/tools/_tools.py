...
class CreateIncWithCI(sys_id):

    cmdb_ci_sys_id: str =''

if __name__ == "__main__":
    if len(sys.argv) > 1:
        sys_id : str = sys.argv[1]
        print(f"Hello, {sys_id}!")
    else:
        print("Please provide a name as an argument.")

        parser = argparse.ArgumentParser(description="A script that takes sys_id.")
        parser.add_argument("sysId", type=str, help="The sys id of ci.")

        args= parser.parse_args()
        cmdb_ci_sys_id = args.sysId
message_json = {
        "impact": 2,
        "priority": 3,
